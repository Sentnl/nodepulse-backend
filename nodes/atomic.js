  ...
  try {
    ...
    try {
      const [collectionsResponse, templatesResponse, assetsResponse, marketAssetsResponse] = await Promise.all([
        ky.get(formatUrl(node.url, '/atomicassets/v1/collections/kogsofficial'), { timeout: false, signal: controller.signal }).json(),
        ky.get(formatUrl(node.url, '/atomicassets/v1/templates?collection_name=kogsofficial&has_assets=true&page=1&limit=1&order=desc&sort=created'), { timeout: false, signal: controller.signal }).json(),
        ky.get(formatUrl(node.url, '/atomicassets/v1/assets?page=1&limit=1&order=desc&sort=asset_id'), { timeout: false, signal: controller.signal }).json(),
        ky.get(formatUrl(node.url, '/atomicmarket/v1/assets?owner=sentnlagents&limit=1'), { timeout: false, signal: controller.signal }).json()
      ]);
      ...
      if (collectionsData.success && templatesData.success && assetsData.success) {
        ...
        if (templateId && assetId) {
          ...
          const [templateResponse, assetResponse] = await Promise.all([
            ky.get(formatUrl(node.url, `/atomicassets/v1/templates/kogsofficial/${templateId}`), { timeout: false, signal: controller.signal }).json(),
            ky.get(formatUrl(node.url, `/atomicassets/v1/assets/${assetId}`), { timeout: false, signal: controller.signal }).json()
          ]);
      ...
      if (marketAssetsData.success && marketAssetsData.data.length > 0) {
        ...
        try {
          const marketAssetResponse = await ky.get(formatUrl(node.url, `/atomicmarket/v1/assets/${marketAssetId}`), { timeout: false, signal: controller.signal }).json();
      ...
      if (error.name === 'AbortError') {
        console.log(`Node ${node.url} check timed out after ${TIMEOUT_DURATION}ms`);
        return false;
      }
      throw error;
    }
